...
public class ExamServiceImpl implements ExamService {
    ...
    public void endExam(Long id) {
        ...
        // 发送WebSocket通知
        try {
            examWebSocketService.sendExamEnded(e.getId(), e.getTitle());
            log.info("考试结束WebSocket通知已发送，考试ID: {}, 标题: {}", e.getId(), e.getTitle());
        } catch (Exception ex) {
            log.error("发送考试结束WebSocket通知失败，考试ID: {}", e.getId(), ex);
        }
    }
    ...
    public void reportSwitchBlur(Long examRecordId) {
        User u = me();
        if (!RoleEnum.STUDENT.name().equals(u.getRole())) {
            throw new BizException(ErrorCode.FORBIDDEN, "仅学生可上报");
        }
        ExamRecord record = examRecordMapper.selectById(examRecordId);
        if (record == null) {
            throw new BizException(ErrorCode.NOT_FOUND, "考试记录不存在");
