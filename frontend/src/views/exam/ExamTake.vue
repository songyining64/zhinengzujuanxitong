      ...
      <el-affix :offset="0">
        <div class="exam-toolbar">
          ...
          <div class="toolbar-right">
            <span class="meta">进度 {{ answeredCount }}/{{ questions.length }}</span>
            ...
            <el-button :loading="saving" @click="manualSave">保存草稿</el-button>
        ...
        <el-card class="answer-sheet" shadow="never">
          <template #header>
            <div class="sheet-head">
              <span>答题卡</span>
              <span class="sheet-meta">未答 {{ unansweredCount }}</span>
            </div>
          </template>
          <div class="sheet-grid">
            <button
              v-for="q in questions"
              :key="`sheet-${q.questionId}`"
              type="button"
              class="sheet-item"
              :class="{ done: isAnswered(q.questionId) }"
              @click="scrollToQuestion(q.orderNo)"
            >
              {{ q.orderNo }}
            </button>
          </div>
...
const answeredCount = computed(() => {
  ...
  for (const q of questions.value) {
    if (isAnswered(q.questionId)) n += 1;
  }
  return n;
});
...
function parseOptions(json: string | null): string[] {
  ...
  if (lines.length <= 1) {
    const oneLine = lines[0] || raw;
    // 兼容 "A xxx B xxx C xxx D xxx" 这类单行录入
    const chunks = oneLine.match(/[A-Ha-h][\.\)、\s]*[\s\S]*?(?=(?:\s+[A-Ha-h][\.\)、\s]+)|$)/g);
    if (chunks && chunks.length > 1) {
      lines = chunks.map((s) => s.trim()).filter(Boolean);
    }
  }
  return lines.map((line) => line.replace(/^[A-Ha-h][\.\)、\s]+/, '').trim()).filter(Boolean);
}
...
function scrollToQuestion(orderNo: number) {
  const el = document.getElementById(`q-${orderNo}`);
  if (!el) return;
...
watch(remainSeconds, (v) => {
  if (v > 0) return;
  if (autoSubmitting.value) return;
  if (!summary.value || summary.value.recordStatus !== 'IN_PROGRESS') return;
  autoSubmitting.value = true;
  ElMessage.warning('考试时间到，系统正在自动交卷');
